...
-- Stash 1
RegisterNetEvent("police:OpenEvidence")
AddEventHandler('police:OpenEvidence', function()
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
            local pos = GetEntityCoords(PlayerPedId())
...
-- Personal Stash
-- CreateThread(function()
RegisterNetEvent('police:openPerStash')
AddEventHandler('polce:openPerStash', function()
    Wait(1000)
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
...
-- Fingerprint
RegisterNetEvent('fingerprint')
AddEventHandler('fingerprint', function()
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "police" then
            local pos = GetEntityCoords(PlayerPedId())
                    if onDuty then
