    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if #(pos - vehpos) < 2.5 then
            ...
            if (vehLockStatus > 0) then
                ...
                exports['qb-lock']:StartLockPickCircle(3, 50, success)
            end
        end
    end
end
...
local function RobVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", "Försöker råna..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random()
        if chance <= Config.RobberyChance then
            ...
            QBCore.Functions.Notify('Du fick bilnycklar!', 'success')
            Wait(10000)
            IsRobbing = false
        else
            PoliceCall()
            ...
            TaskReactAndFleePed(target, PlayerPedId())
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            QBCore.Functions.Notify('Någon ringde polisen!', 'error')
            Wait(10000)
            IsRobbing = false
        end
    end)
...
RegisterNetEvent('vehiclekeys:client:GiveKeys', function(target)
    ...
    if vehicles then
        ...
        QBCore.Functions.Notify('Du har inga bilnycklar att ge', 'error')
    end
end)
