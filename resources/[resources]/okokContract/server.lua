...
AddEventHandler('okokContract:changeVehicleOwner', function(data)
	...
	exports.oxmysql.execute('SELECT * FROM player_vehicles WHERE citizenid = @identifier AND plate = @plate', {
		...
		if Config.RemoveMoneyOnSign then
			...
			if result[1] ~= nil  then
				if bankMoney >= vehicle_price then
					exports['oxmysql']:execute('UPDATE player_vehicles SET citizenid = @target WHERE citizenid = @owner AND plate = @plate', {
